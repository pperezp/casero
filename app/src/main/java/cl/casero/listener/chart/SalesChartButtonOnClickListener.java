package cl.casero.listener.chart;
...
import cl.casero.ChartActivity;
import cl.casero.R;
...
public class SalesChartButtonOnClickListener implements View.OnClickListener {
    ...
    public SalesChartButtonOnClickListener(){
    ...
    private void loadComponents() {
        startMonthSpinner = (Spinner) activity.findViewById(R.id.startMonthSpinner);
        endMonthSpinner = (Spinner) activity.findViewById(R.id.endMonthSpinner);
        startYearSpinner = (Spinner) activity.findViewById(R.id.startYearSpinner);
        endYearSpinner = (Spinner) activity.findViewById(R.id.endYearSpinner);
        barChart = (BarChart) activity.findViewById(R.id.barChart);

        Util.loadYears(activity , startYearSpinner);
        Util.loadYears(activity, endYearSpinner);
    }
    ...
    public void onClick(View view) {
        ...
        // TODO: Arreglar este algoritmo, muchos for similares
        for(int year = startDate.getYear(); year <= endDate.getYear(); year++){
            if(startDate.getYear() == endDate.getYear()){// si es el mismo año, recorro normal
                for(int month = startDate.getMonth(); month <= endDate.getMonth(); month++){
                    monthlyStatistic = statisticsService.getMonthlyStatistic(month, year);
                ...
                if(year == startDate.getYear()){
                    for(int month = startDate.getMonth(); month <= 12; month++) {
                        monthlyStatistic = statisticsService.getMonthlyStatistic(month, year);
                        sales.add(new BarEntry(xCount, monthlyStatistic.getSalesCount()));
                        payments.add(new BarEntry(xCount, monthlyStatistic.getPaymentsCount()));
                        xCount++;
